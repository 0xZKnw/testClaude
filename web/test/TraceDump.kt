 ...
 * bot itself, which walks the bot as well.
 */
fun main() {
    ...
    fun snapshot(tag: String, e: UnoEngine): String {
        ...
        return "$tag|${e.turn}|${e.phase}|${e.activeColor}|${e.pendingDraw}|${e.pendingType}|" +
            "${e.deckCount()}|$hands|${e.top().id}|${e.direction}|${e.winner}|$seat0|${e.event}"
    }

    for (players in 2..5) {
        for (seed in 1..30) {
            // ---- policy A: always the lowest legal card
            var e = UnoEngine(Random(seed.toLong()), players)
            e.startRound(seed % players)
            out.append(snapshot("A$players/$seed deal", e)).append('\n')
            var guard = 0
            while (e.phase != Phase.GAME_OVER && guard++ < 4000) {
                e.autoAdvance()
                out.append(snapshot("A$players/$seed auto", e)).append('\n')
                ...
                val action = when {
                    ...
                    e.phase == Phase.DECIDE_AFTER_DRAW -> { e.pass(seat); "pass" }
                    else -> { e.draw(seat); "draw" }
                }
                out.append(snapshot("A$players/$seed $action", e)).append('\n')
            }

            // ---- policy B: the bot, every level
            val rng = Random(seed.toLong() * 31)
            e = UnoEngine(Random(seed.toLong()), players)
            e.startRound(seed % players)
            val levels = (0 until players).map { Difficulty.entries[it % Difficulty.entries.size] }
            guard = 0
            while (e.phase != Phase.GAME_OVER && guard++ < 4000) {
                e.autoAdvance()
                if (e.phase == Phase.GAME_OVER) break
                ...
                out.append(snapshot("B$players/$seed $action", e)).append('\n')
            }
        }
    }
    print(out)
